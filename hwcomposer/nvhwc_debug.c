...
static int dump_display(struct hwc_context_t *ctx,
    ...
    for (ii = 0; ii < num_windows; ii++) {
        if ((int)ii == dpy->fb_index) {
            DUMP("\tWindow %d (phys %d caps %d blend 0x%x xform 0x%x): "
                 ...
                 dpy->caps.window_cap[conf->overlay[ii].window_index],
                 conf->overlay[ii].blend, conf->overlay[ii].transform,
                 (dpy->composite.list.nLayers && dpy->composite.scratch) ?
                     "scratch": "fb",
                 dpy->composite.list.nLayers ?
                     dpy->composite.list.nLayers : dpy->fb_layers,
            ...
            if (ctx->legacy) {
                if (dpy->map[ii].index == ctx->video_out_index) {
                    DUMP(" (video)");
                }
#if 0
                else if (ctx->hotplug.cached.connected &&
                           ctx->hdmi_mode == HDMI_MODE_VIDEO) {
                    if (dpy->map[ii].index == ctx->cc_out_index) {
                        DUMP(" (cc)");
                    } else if (dpy->map[ii].index == ctx->osd_out_index) {
                        DUMP(" (osd)");
                    }
                }
#endif
...
void hwc_dump(hwc_composer_device_t *dev, char *buff, int buff_len)
{
    ...
    struct hwc_display_t *primary = (struct hwc_display_t *)
        &ctx->displays[HWC_DISPLAY_PRIMARY];
    struct hwc_display_t *external = (struct hwc_display_t *)
        &ctx->displays[HWC_DISPLAY_EXTERNAL];
    int len = 0;

    DUMP("  Nvidia HWC:\n");
    if (ctx->legacy) {
        DUMP("    Legacy mode enabled\n");
        DUMP("    Panel orientation: %d\n",
             rotation_from_transform(ctx->panel_transform));
    }
    ...
    DUMP("    Currently compositing %d layers into a %s\n",
         primary->composite.list.nLayers,
        primary->composite.scratch ? "scratch buffer" : "framebuffer");
