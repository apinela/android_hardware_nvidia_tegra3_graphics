...
typedef struct NvRmStreamRec
{
    ...
    // Time, in milliseconds, to wait for a submit in the stream to complete.
    // As part of a flush to the kernel, a buffer is allowed to run for this
    // amount of time, before it's consider hung. At that point, futher submits
    // aren't issued and return as errors for this context.
    NvU32 SubmitTimeout;
...
#define NVRM_STREAM_PUSH_U(pCurrent, u1) \
    do { ((pCurrent)++)->u = (u1); } while (0)

#define NVRM_STREAM_PUSH_I(pCurrent, i1) \
    do { ((pCurrent)++)->i = (i1); } while (0)

#define NVRM_STREAM_PUSH_F(pCurrent, f1) \
    do { ((pCurrent)++)->f = (f1); } while (0)
...
#define NVRM_STREAM_PUSH_RELOC(pStream, pCurrentArg, hMemArg, OffsetArg) \
    NVRM_STREAM_PUSH_RELOC_SHIFT(pStream, pCurrentArg, \
        hMemArg, OffsetArg, 0)
